...
/**
* Equals, test for data equivalences
...
void ByteBuffer::putSort(uint16_t value, uint32_t index)
{
...
{
	int iLen = (szName == NULL) ? 0 : strlen(szName);
	putInt(iLen);
	for (int i = 0; i < iLen; i++)
		putChar(szName[i]);
}
...
void ByteBuffer::printInfo()
{
	...
	OutputDebugString(CvString::format("ByteBuffer %s Length: %u. Info Print\n", name.c_str(), length).c_str());
}

void ByteBuffer::printAH()
{
	uint32_t length = buf.size();
	CvString szBuffer = CvString::format("ByteBuffer %s Length: %u. ASCII & Hex Print\n", name.c_str(), length);
	for (uint32_t i = 0; i < length; i++)
		szBuffer.append(CvString::format("0x%02x ", buf[i]));
	szBuffer.append("\n");
	for (uint32_t i = 0; i < length; i++)
		szBuffer.append(CvString::format("%c ", buf[i]));
	szBuffer.append("\n");
	OutputDebugString(szBuffer.c_str());
}

void ByteBuffer::printAscii()
{
	uint32_t length = buf.size();
	CvString szBuffer = CvString::format("ByteBuffer %s Length: %u. ASCII Print\n", name.c_str(), length);
	for (uint32_t i = 0; i < length; i++)
	{
		szBuffer.append(CvString::format("%c ", buf[i]));
	}
...
{
	uint32_t length = buf.size();
	CvString szBuffer = CvString::format("ByteBuffer %s Length: %u. Hex Print\n", name.c_str(), length);
	for (uint32_t i = 0; i < length; i++)
		szBuffer.append(CvString::format("0x%02x ", buf[i]));
	szBuffer.append("\n");
	OutputDebugString(szBuffer.c_str());
...
{
	uint32_t length = buf.size();
	OutputDebugString(CvString::format("ByteBuffer %s Length: %u Read Pos: %u Wirte Pos %u\n", name.c_str(), length, rpos, wpos).c_str());
}
#endif
