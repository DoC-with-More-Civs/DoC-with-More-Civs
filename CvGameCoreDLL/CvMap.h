...
/*********************************/
	...
	// Viewports are owned by theri underlying maps
...
/*******************************/
	...
	inline int plotNumINLINE_checked(int iX, int iY) const
	{
		...
		int iMapX = coordRange(iX, getGridWidthINLINE(), isWrapYINLINE());
		int iMapY = coordRange(iY, getGridHeightINLINE(), isWrapYINLINE());
		return (isPlotINLINE(iMapX, iMapY) ? plotSorenINLINE(iMapX, iMapY) : -1);
	}
#endif
	...
	bool isWrapX();																							// Exposed to Python
#ifdef _USRDLL
...
#endif
	bool isWrapY();																							// Exposed to Python
#ifdef _USRDLL
...
/************************************************************************************************/
	// Plot danger cache
	void invalidateIsActivePlayerNoDagerCache();
...
/************************************************************************************************/
	...
	//Afforess - MP Resync
	void resync(bool bWrite, ByteBuffer* pStream);
