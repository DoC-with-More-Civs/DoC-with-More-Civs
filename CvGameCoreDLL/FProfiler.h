...
// modified: wunshare
//	- ProfileSample 支持多线程
//	- 使用宏定义 USE_INTERNAL_PROFILER 来打开
//	- 在 CvGameCoreDLL.h 中声明
//	- 在 CvGameCoreDLL.cpp 中实现
//---------------------------------------------------------------------------------------------------------------------
...
#define MAX_PROFILED_THREADS	(8)	// Profile 线程数 
...
#ifdef USE_INTERNAL_PROFILER
	int				ProfileInstance[MAX_PROFILED_THREADS + 1];	// # of times ProfileBegin Called
	LONG			EntryCount[MAX_PROFILED_THREADS + 1];

	// All samples this frame added together for the main thread only
	LARGE_INTEGER	MainThreadAccumulator[MAX_PROFILED_THREADS + 1]; 
	// All samples this frame added together
	LARGE_INTEGER	Accumulator[MAX_PROFILED_THREADS + 1];

	// Time taken by all children
	LARGE_INTEGER	ChildrenSampleTime[MAX_PROFILED_THREADS + 1];
	// Time taken by all descendants flagged as contributing to altrernate set
	LARGE_INTEGER	AlternateSampleSetTime[MAX_PROFILED_THREADS + 1];
#else
...
#define PROFILE_END_CONDITIONL(result)\
	if (result)\
...
#else // !USE_INTERNAL_PROFILER
#define PROFILE_SET_COUNT(name,value)	;
#define PROFILE(name)\
	static ProfileSample(name);\
	CProfileScope ProfileScope(&sample);
...
#define PROFILE_BEGIN_CONDITIONL(name1,name2)
#define PROFILE_END_CONDITIONL(result)
