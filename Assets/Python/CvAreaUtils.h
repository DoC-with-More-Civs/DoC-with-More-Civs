...
typedef struct CvRectangle {
	std::pair<int, int> m_tBL;
	std::pair<int, int> m_tTR;
	
	CvRectangle(std::pair<int, int> tBL, std::pair<int, int> tTR)
		m_tBL(tBL), m_tTR(tTR)
	{
	}
	...
	int left() { m_tBL.first; }
	int bottom() { m_tBL.second; }
	int right() { m_tTR.first; }
	int top() { m_tTR.second; }
...
public:
	static bool isReborn(PlayerTypes iPlayer);
	
	void CvAreaUtils::getOrElse(std::map<int, std::vector<std::pair<int, int> > >& dDictionary, int key, std::vecotr<std::pair<int, int> >& defaultVal, std::vecotr<std::pair<int, int> >& retVal);
	
	void getArea(PlayerTypes iPlayer, CvRectangle& tRectangle, std::map<int, std::vector<std::pair<int, int> > >& dExceptions, boost::tribool bReborn = None, std::map<int, CvRectangle>& dChangedRectangle = 0, std::map<int, std::vector<int, int> > >& dChangedExceptions = 0);
	...
	void getBirthArea(PlayerTypes iPlayer, std::vector<std::pair<int, int> >& tRet);
	static void getBirthRectangle(PlayerTypes iPlayer, CvRectangle& birthRectangle,boost::tribool bExtended = None);
	void getBirthExceptions(PlayerTypes iPlayer, std::vector<std::pair<int, int> >& tRet);
	
	void getCoreArea(PlayerTypes iPlayer, std::vector<std::pair<int, int> >& tRet, boost::tribool bReborn = None);
	void getNormalArea(PlayerTypes iPlayer,std::vector<std::pair<int, int> >& tRet, boost::tribool bReborn = None);
	void getBroaderArea(PlayerTypes iPlayer, boost::tribool bReborn = None);
	void getRespawnArea(PlayerTypes iPlayer, std::vector<std::pair<int, int> >& tRet);
	void getRebirthArea(PlayerTypes iPlayer, std::vector<std::pair<int, int> >& tRet);
...
public:
	std::vector<std::pair<int, int> >		m_lPeakExceptions;
	
	// ### Capitals ###
	std::vector< std::pair<int, int> >   	m_tCapitals;
	std::map<int, std::pair<int, int> > 	m_dChangedCapitals;
	std::map<int, std::pair<int, int> >	    m_dNewCapitals;
	std::map<int, std::pair<int, int> >     m_dRespawnCapitals;
	
	// ### Birth Area ###	
	std::vector<CvRectangle>				m_tBirthArea;
	std::map<int, CvRectangle>				m_dChangedBirthArea;
	std::map<int, std::vector<std::pair<int, int> > > m_dBirthAreaExceptions;
	
	// ### Core Area ###	
	std::vector<CvRectangle> 				m_tCoreArea;
	std::map<int, CvRectangle> 				m_dChangedCoreArea;
	std::map<int, std::vector<int, int> > > m_dCoreAreaExceptions;
	std::map<int, std::vector<int, int> > > m_dChangedCoreAreaExceptions;
	
	// ### Normal Area ###				
	std::vector<CvRectangle> 				m_tNormalArea;	
	std::map<int, CvRectangle> 				m_dChangedNormalArea;
	std::map<int, std::vector<std::pair<int, int> > > m_dNormalAreaExceptions;	
	std::map<int, std::vector<std::pair<int, int> > >	m_dChangedNormalAreaExceptions;	
	
	// ### Broader Area ###		
	std::vector<CvRectangle> 				m_tBroaderArea;
	std::map<int, CvRectangle> 				m_dChangedBroaderArea;	
	
	// ### Respawn area ###	
	std::map<int, CvRectangle> 				m_dRespawnArea;	
	std::map<int, std::vector<std::pair<int, int> > > 	m_dRespawnAreaExceptions;	
	
	// ### Rebirth area ###	
	std::map<int, std::pair<int, int> >     m_dRebirthPlot;
	std::map<int, CvRectangle>				m_dRebirthArea;
	std::map<int, std::vector<std::pair<int, int> > > m_dRebirthAreaExceptions;
private:
	CvAreaUtils();
};
...
using namespace boost::assign;	// 列表初始化
...
void CvAreaUtils::getOrElse(std::map<int, std::vector<std::pair<int, int> > >& dDictionary, int key, std::vecotr<std::pair<int, int> >& defaultVal, std::vecotr<std::pair<int, int> >& retVal)
{
	if (dDictionary && VAL_IN(key, dDictionary)) 
	{
...
void CvAreaUtils::getArea(PlayerTypes iPlayer, CvRectangle& tRectangle, std::map<int, std::vector<std::pair<int, int> > >& dExceptions, std::vector<std::pair<int, int>& tRet, boost::tribool bReborn, std::map<int, CvRectangle>& dChangedRectangle, std::map<int, std::vector<int, int> > >& dChangedExceptions)
{
	if (boost::indeterminate(bReborn)) bReborn = isReborn(iPlayer);
	CvRectangle tPlot = tRectangle;
	
	std::vector<std::pair<int, int> > lExceptions; 
	getOrElse(dExceptions, iPlayer, NULL, lExceptions)
	
	if (bReborn)
	{
		if (VAL_IN(iPlayer, dChangedRectangle))
		{
			tPlot = dChangedRectangle[iPlayer]; 
			getOrElse(dChangedRectangle, iPlayer, NULL, lExceptions)
		}
	}
	int left = tPlot.left();
	int bottom = tPlot.bottom();
	int right = tPlot.right();
	int top = tPlot.top();;
	...
	for (int iX = left; iX < right + 1; iX++)
	{
		...
		{
			std::pair<int, int> coord = std::make_pair(iX, iY);
			if (!VAL_IN(coord, lExceptions))
				tRet.append(coord);
		}
	}
}
...
{
	if (!bReborn) bReborn = isReborn(iPlayer);
	if (bReborn && VAL_IN(iPlayer, m_dChangedCapitals))
	{
		std::pair<int, int> tPlot = m_dChangedCapitals[iPlayer];
		return GC.getMapINLINE().plot(tPlot->first, tPlot->second);
	}
	std::pair<int, int> tPlot = m_tCapitals[iPlayer];
	return GC.getMapINLINE().plot(tPlot->first, tPlot->second);
}

CvPlot* CvAreaUtils::getRespawnCapital(PlayerTypes iPlayer, boost::tribool bReborn)
{
	if (VAL_IN(iPlayer, m_dRespawnCapitals)) 
	{
		std::pair<int, int> tPlot = m_dChangedCapitals[iPlayer]
		return GC.getMapINLINE().plot(tPlot->first, tPlot->second);
	}
...
{
	if (VAL_IN(iPlayer, m_dNewCapitals))
	{
		std::pair<int, int> tPlot = m_dNewCapitals[iPlayer];
		return GC.getMapINLINE().plot(tPlot->first, tPlot->second);
	}
	return getRespawnCapital(iPlayer, bReborn);
}

void CvAreaUtils::getBirthArea(PlayerTypes iPlayer, std::vector<std::pair<int, int> >& tRet)
{
	getArea(iPlayer, m_tBirthArea, m_dBirthAreaExceptions, tRet);
}

void CvAreaUtils::getBirthRectangle(PlayerTypes iPlayer, CvRectangle& tRet, boost::tribool bExtended)
{
	if (!bExtended) bExtended = isExtendedBirth(iPlayer);
	if (VAL_IN(iPlayer, m_dChangedBirthArea) && bExtended)
		tRet = m_dChangedBirthArea[iPlayer];
	else
		tRet = m_tBirthArea[iPlayer];
}

void CvAreaUtils::getBirthExceptions(PlayerTypes iPlayer, std::vector<std::pair<int, int> >& tRet)
{
	if (VAL_IN(iPlayer, m_dBirthAreaExceptions))
		tRet = m_dBirthAreaExceptions[iPlayer];
	else
		tRet.clear();
}

void CvAreaUtils::getCoreArea(PlayerTypes iPlayer, std::vector<std::pair<int, int> >& tRet, boost::tribool bReborn)
{
	getArea(iPlayer, m_tCoreArea, m_dCoreAreaExceptions, tRet, bReborn, m_dChangedCoreArea, m_dChangedCoreAreaExceptions);
}

void CvAreaUtils::getNormalArea(PlayerTypes iPlayer,std::vector<std::pair<int, int> >& tRet, boost::tribool bReborn)
{		
	getArea(iPlayer, m_tNormalArea, m_dNormalAreaExceptions, tRet, bReborn, m_dChangedNormalArea, m_dChangedNormalAreaExceptions);			
}
				
void CvAreaUtils::getBroaderArea(PlayerTypes iPlayer, boost::tribool bReborn)
{				
	getArea(iPlayer, m_tBroaderArea, NULL, tRet, bReborn, m_dChangedBroaderArea);
}
				
void CvAreaUtils::getRespawnArea(PlayerTypes iPlayer, std::vector<std::pair<int, int> >& tRet)
{				
	if (VAL_IN(iPlayer, m_dRespawnArea)  
		getArea(iPlayer, dRespawnArea, NULL, tRet);
	else
		getNormalArea(iPlayer, tRet);	
}
				
void CvAreaUtils::getRebirthArea(PlayerTypes iPlayer, std::vector<std::pair<int, int> >& tRet)
{				
	if (VAL_IN(iPlayer, m_dRebirthArea)
		getArea(iPlayer, m_dRebirthArea, m_dRebirthAreaExceptions, tRet);			
	else
...
void CvAreaUtils::updateCore(PlayerTypes iPlayer)
{
	...
	for (int iX = 0; iX < iWorldX; iX++)
	{
		...
		{	
			std::pair<int, int> tPlot = std::make_pair(iX, iY);
			CvPlot* plot = GC.getMapINLINE().plot(iX, iY);
			if (plot->isWater() || 
				(pPlot->isPeak() && !VAL_IN(tPlot, m_lPeakExceptions)) 
				continue;
...
{
	for (int iLoopPlayer = 0; iLoopPlayer < iNumPlayers; iLoopPlayer++)
	{
		if (iLoopPlayer == iPlayer) continue;
		if (pPlot->isCore(iLoopPlayer))
			return true;
	}
	return false;
}
...
CvAreaUtils::CvAreaUtils()
{
	...
	insert(m_dNormalAreaExceptions)				
		...
		( iArabia, 		(map_list_of(84, 35)(84, 36)(84, 37)(85, 35)(88, 45)(88, 46)(89, 45)(89, 46)(90, 44)(90, 45)(90, 46)(91, 45)(91, 46)(92, 45)(92, 46)(93, 44)(93, 45)(94, 43)(94, 44)(94, 45)(94, 46)(95, 43)(95, 45)(95, 46)(96, 43)(96, 44)(96, 45)(96, 46)) )iSpain, (map_list_of(49, 44)(49, 43)(49, 42)(49, 41)) )				
